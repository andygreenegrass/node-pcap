...
// TODO: this code was generalised to find IPv4 upon layers of encapsulations.
// At the moment the implementation loops through the layers upon each state,
// which can be further improved by assuming that the packet is IPv4 TCP.
// However, that will significantly change the API which may introduce some
// breaking changes.

function getIPV4Packet(packet) {
    while (packet && !(packet instanceof IPv4)) {
...
TCPTracker.prototype.track_packet = function (packet) {
    // Silently ignore any non IPv4 TCP packets user should filter these out
    // with their pcap filter, but oh well.

    var ip = getIPV4Packet(packet);
    ...
    // Need to track at least one packet before we emit this new session,
    // otherwise nothing will be initialized.
